function dy = wendModel_ode(t, y, pt, P, excs, i)   
p = y(1:8:end);
e = y(2:8:end);
s = y(3:8:end);
f = y(4:8:end);
...
%         interafferent(n) = interafferent(n) + P.Cij(n, m) * sigmoid(excs(m, i - D_ndt(n,m)));
    end
end
    dy(1:8:end) = pp;
    dy(2:8:end) = ee;
    dy(3:8:end) = ss;
    dy(4:8:end) = ff;
    dy(5:8:end) = P.A * P.a * sigmoid(e - s - f) - 2 * P.a * pp - P.a ^ 2 * p;
